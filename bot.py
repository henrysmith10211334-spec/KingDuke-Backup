...
async def speedups(interaction: discord.Interaction, image: discord.Attachment):
    ...
    if REPORT_CHANNEL_ID is None:
        await interaction.response.send_message(
            embed=make_embed("⚠️ The report channel has not been set yet.", discord.Color.yellow()),
            ephemeral=True
        )
        return
    ...
    if sleeping:
        await interaction.response.send_message(
            embed=make_embed("❌ Bot is under maintenance.", discord.Color.red()),
            ephemeral=True
        )
        return
    ...
    try:
        ...
        if existing_row > -1:
            ...
            await interaction.followup.send(embed=make_embed("✅ Report sent!", discord.Color.green()), ephemeral=True)
    except Exception:
...
async def setreportchannel(interaction: discord.Interaction, channel: discord.TextChannel):
    ...
    await interaction.response.send_message(
        embed=make_embed(f"✅ Reports now allowed only in <#{REPORT_CHANNEL_ID}>.", discord.Color.green()),
        ephemeral=True
    )
